...
def test_consecutive(backend: str):
    with pyvirtualcam2.Camera(width=1280, height=720, fps=20, backend=backend) as cam:
...
def test_multiple_cameras_are_supported(backend: str):
    cam1 = pyvirtualcam2.Camera(width=1280, height=720, fps=20, backend=backend)
    frame = np.zeros((cam1.height, cam1.width, 3), np.uint8) # RGB
    cam1.send(frame)

    cam2 = pyvirtualcam2.Camera(width=640, height=480, fps=20, backend=backend)
...
def test_select_camera_device(backend: str):
    ...
    with pyvirtualcam2.Camera(width=1280, height=720, fps=20, device=device) as cam:
        frame = np.zeros((cam.height, cam.width, 3), np.uint8) # RGB
        cam.send(frame)
...
def test_alternative_pixel_formats(backend: str):
    ...
    if backend == 'unitycapture':
        with pyvirtualcam2.Camera(width=1280, height=720, fps=20, fmt=PixelFormat.RGBA, backend=backend) as cam:
            check_native_fmt(cam)
            cam.send(np.zeros((cam.height, cam.width, 4), np.uint8))
    
    with pyvirtualcam2.Camera(width=1280, height=720, fps=20, fmt=PixelFormat.BGR, backend=backend) as cam:
        check_native_fmt(cam)
        cam.send(np.zeros((cam.height, cam.width, 3), np.uint8))
    
    with pyvirtualcam2.Camera(width=1280, height=720, fps=20, fmt=PixelFormat.GRAY, backend=backend) as cam:
        check_native_fmt(cam)
        cam.send(np.zeros((cam.height, cam.width), np.uint8))

    with pyvirtualcam2.Camera(width=1280, height=720, fps=20, fmt=PixelFormat.I420, backend=backend) as cam:
        check_native_fmt(cam)
        cam.send(np.zeros(cam.height * cam.width + cam.height * (cam.width // 2), np.uint8))

    with pyvirtualcam2.Camera(width=1280, height=720, fps=20, fmt=PixelFormat.NV12, backend=backend) as cam:
        check_native_fmt(cam)
        cam.send(np.zeros(cam.height * cam.width + cam.height * (cam.width // 2), np.uint8))
    
    with pyvirtualcam2.Camera(width=1280, height=720, fps=20, fmt=PixelFormat.YUYV, backend=backend) as cam:
        check_native_fmt(cam)
        cam.send(np.zeros(cam.height * cam.width * 2, np.uint8))
    
    with pyvirtualcam2.Camera(width=1280, height=720, fps=20, fmt=PixelFormat.UYVY, backend=backend) as cam:
        check_native_fmt(cam)
        cam.send(np.zeros(cam.height * cam.width * 2, np.uint8))
...
def test_sleep_until_next_frame():
    target_fps = 20
    with pyvirtualcam2.Camera(width=1280, height=720, fps=target_fps) as cam:
        frame = np.zeros((cam.height, cam.width, 3), np.uint8) # RGB
        for _ in range(100):
            cam.send(frame)
